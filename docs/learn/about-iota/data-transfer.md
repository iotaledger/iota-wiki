...
IOTA is the only Cryptocurrency and DLT Technology that is capable of transferring Data totally free, fast, immutable, unforgeable, and secure. 
These unique feautures open up a wide array of use cases that normal cryptocurrencies can never support.
...
A message is an object consisting of information which is broadcasted in the Tangle.
...
If a node decides that a message is valid it will sent this messages to it's neighbors using the gossip protocol. Every neighbor that receives the message transfers it again to its neighbors and so on. Very quickly every other node in the network sees the message and has the same information and the same knowledge of the "State" of the network at a given time.
...
A message containing only Data here is called an **indexation payload**. This message can be sent without the need for a signature and can contain a specific amount of any kind of data the user wants to send.
