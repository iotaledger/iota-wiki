...
- Mana can also be allotted to a chosen account as [Block Issuance Credits (BIC)](#block-issuance-credits-bic). Note that this is not the default behavior of the wallet, but it is technically possible to do so (for example, one might want to allot extra _BIC_ to issue data blocks in the future).
