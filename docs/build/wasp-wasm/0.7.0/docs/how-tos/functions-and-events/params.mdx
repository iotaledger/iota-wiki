...
[`state`](../schema-tool/state.mdx) subsection field definitions with one addition, the field type can be
followed by a question mark to indicate that the parameter is optional.

The [Schema Tool](../schema-tool/usage.mdx) will automatically generate an immutable structure with
...
Note that the [Schema Tool](../schema-tool/usage.mdx) will also generate a mutable version of the
