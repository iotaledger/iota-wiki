...
- the [State](../schema-tool/state.mdx) map, which holds all state storage values
...
The [Schema Tool](../schema-tool/usage.mdx) is able to build a more complex, JSON-like data structure
...
The [Schema Tool](../schema-tool/usage.mdx) will make sure that the type-safe code that it generates
...
The [Schema Tool](../schema-tool/usage.mdx) automatically generates code that allows the user to navigate
...
map, keys are always of the same data type. The root maps ([State](../schema-tool/state.mdx),
...
[WasmLib Data Types](../../reference/data-types/types.mdx).
