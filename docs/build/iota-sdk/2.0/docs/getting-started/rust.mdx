...
<TabItem value="chocolatey" label="chocolatey" default>
...
To start using the IOTA SDK in your Rust project, you can include it as a dependency in your project's `Cargo.toml` by running:

```bash
cargo add iota-sdk@2.0.0
```

Or you can include the following line under `dependencies` in your `Cargo.toml`:
...
3. Checkout the `2.0` branch:

 ```bash
 git checkout 2.0
    ```

4. Build the SDK by running the following:
...
The SDK requires an asynchronous runtime, like [Tokio](https://tokio.rs/). Add it to your dependencies by running:

```bash
cargo add tokio --features=full
```

or by adding it to your dependencies:
...
```rust reference
https://github.com/iotaledger/iota-sdk/blob/2.0/sdk/examples/client/getting_started.rs#L10-L23
```

### Wallet

To use the Wallet module, you need to create a `Wallet`.
For this example to work `features = ["stronghold", "storage"]` is needed in the Cargo.toml import.

```rust reference
https://github.com/iotaledger/iota-sdk/blob/2.0/sdk/examples/wallet/getting_started.rs#L11-L57
```
...
You can use the provided code [examples](https://github.com/iotaledger/iota-sdk/tree/2.0/sdk/examples) to get
