...
description: The specification for an embeddedable time-based revocation method - `RevocationTimeframe2024`.
...
in order to address it's linkability security concerns, at least when used in combination with selectively-discolable (SD-able) VCs.

## Introduction

`RevocationBitmap2022` allows for a simple and straightforward way for an issuer to invalidate an issued VC before it's expiration. This mechanism though
comes with a high risk of linkability, which is especially relavant for bitmaps stored on-chain - where the storage space limits the maximum size for
the revocation bitmap, reducing herd privacy.
...
in which the credential is guaranted to be non-revoked. By having a validity timeframe embedded in the credential's status
the verifier can prove the credential's validity by verifying the credential's signature.

The downside of this mechanism is that the holder of a credential using `RevocationTimeframe2024` has to contact the credential's issuer
to update the signature - i.e. re-issue the credential - at the end of the credential's validity timeframe. Furthermore,
given as how a credential's validity timeframe proves the validity of the credential itself, the updates made by the issuer to the credential's status -
...
issuers should choose a validity timeframe's length with respect to how frequently they expect to change the credential's status;
frequent updates deems shorter validity timeframes.

:::note
A credential's holder does not need to have its credential re-issued at the end of every validity timeframe, but only when
she needs to present the credential to a verifier and the credential's validity timeframe has expired.
:::
...
To allow verifiers to check the status of a [Revocation Timeframe Status](#revocation-bitmap-status), the DID document of the credential issuer MUST contain a [service](https://www.w3.org/TR/did-core/#services) with the following properties:
