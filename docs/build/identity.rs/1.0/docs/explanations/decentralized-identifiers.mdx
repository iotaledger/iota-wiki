...
Lastly, a DID also contains a set of random characters that are unique per identity.
This makes the identity unique and makes sure every identity resolves to a single DID Document.
...
A DID Document mostly contains two important pieces of data: public keys and services.
The public keys can be used to prove ownership over the identity,
...
DIDs become more interesting when you combine them [Verifiable Credentials (VC)](verifiable-credentials.mdx).
...
The Holder controls a copy of this statement and can share it with other parties, the Verifiers,
