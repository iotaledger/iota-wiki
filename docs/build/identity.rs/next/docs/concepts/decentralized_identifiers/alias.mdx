...
The IOTA method uses the IOTA ledger which uses the [unspent transaction output (UTXO) model](/goshimmer/protocol_specification/components/ledgerstate). Also, the features of the [Stardust](/introduction/develop/explanations/what_is_stardust) upgrade are fundamental to the IOTA DID method.
