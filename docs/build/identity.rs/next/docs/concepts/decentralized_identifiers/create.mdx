...
In order to run the examples, a private network (sometimes also called a private Tangle) needs to run locally. You can refer to [Hornet wiki](/hornet/develop/how_tos/private_tangle) for instructions on how to run a private network.
