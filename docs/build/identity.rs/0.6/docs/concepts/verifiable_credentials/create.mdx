...
A [verifiable credential (VC)](./overview.mdx) can represent all information that a physical credential represents, such as a passport or university degree. However, by allowing other parties to cryptographically verify the authorship and _integrity_ of the claims, verifiable credentials can be seen as more tamper-evident and more trustworthy than their physical counterparts.
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/support/v0.6/examples/account/create_vc.rs
```
