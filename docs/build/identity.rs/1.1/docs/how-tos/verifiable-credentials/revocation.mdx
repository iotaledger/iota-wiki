...
### StatusList2021
[StatusList2021](https://www.w3.org/TR/2023/WD-vc-status-list-20230427) offers similar functionalities but in a more flexible way. 
...
can be made available over HTTP (e.g. `http://example.com/credentials/status`) or through the
Interplanetary File System (e.g. `ipfs://QmXDWGdVBhbDoXXzKNMhJk5ejnZgxpMBVzW4EhQaHPD3Mi`).
This flexibility though requires the issuer to manually fetch and update its status list.

Furthermore, credentials can be in any of these three different states:
- _valid_
- _suspended_: the credential won't be temporarily accepted. This status can be reversed.
- _revoked_: the credential isn't valid anymore. This status **cannot** be reversed.

<Tabs groupId="language" queryString>
<TabItem value="rust" label="Rust">

```rust reference
https://github.com/iotaledger/identity.rs/blob/main/examples/1_advanced/8_status_list_2021.rs#L86-L90
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/main/bindings/wasm/examples/src/1_advanced/7_status_list_2021.ts#L72-L76
```
...
and the location at which it will be available (`http://example.com/credentials/status` in this case). After creation, the issuer
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/main/examples/1_advanced/8_status_list_2021.rs#L173
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/main/bindings/wasm/examples/src/1_advanced/7_status_list_2021.ts#L147
```

</TabItem>
</Tabs>

To set the status of a credential the issuer retrieves the status list credential and set the value of the chosen entry index.
This sets the `credentialStatus` of the revoked credential to something similar to the following snippet:
