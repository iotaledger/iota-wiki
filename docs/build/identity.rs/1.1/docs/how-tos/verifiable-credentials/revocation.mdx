...
The size of the revocation list is limited by DLT's size constraints. With the assumption of only one such revocation list
per issuer's DID document, one may expect to be able to handle roughly around 50k entries.
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/examples/0_basic/7_revoke_vc.rs#L167
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/bindings/wasm/examples/src/0_basic/7_revoke_vc.ts#L156
```
...
is stored and fetched, as long as its location can be encoded through an URL. For instance, the status list
...
entries in the status list, in contrast with `RevocationBitmap2022` where the lenght of the list is limited by the DLT's constraints.

Furthermore, `StatusList2021` introduces a new credential state: _suspended_. Suspended credentials are credentials that will not be 
accepted by a validator as _valid_, but that might become valid again in the future. Instead, _revoked_ credentials **cannot** ever
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/examples/1_advanced/8_status_list_2021.rs#L86-L90
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/bindings/wasm/examples/src/1_advanced/7_status_list_2021.ts#L72-L76
```
...
must make the credential available at the chosen URL, so that verifiers may be able to fetch it.

Upon issueing a credential, in order to revoke it or suspend it later on, the issuer sets the `credentialStatus` field, linking
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/examples/1_advanced/8_status_list_2021.rs#L173
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/bindings/wasm/examples/src/1_advanced/7_status_list_2021.ts#L147
```
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/examples/0_basic/7_revoke_vc.rs#L197-L204
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/bindings/wasm/examples/src/0_basic/7_revoke_vc.ts#L192C1-L195
```
...
```rust reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/examples/0_basic/7_revoke_vc.rs
```
...
```ts reference
https://github.com/iotaledger/identity.rs/blob/v1.1.0/bindings/wasm/examples/src/0_basic/7_revoke_vc.ts
```
