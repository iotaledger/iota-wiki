...
When a verifier receives an SD-JWT, it may be desirable to verify that the identity of the presenter matches the holder of the Credential. For that purpose a Key Binding JWT can be used, in which the holder can proof the possession of the private key that corresponds with his public key.
