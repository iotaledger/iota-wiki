...
In the previous sections we showed how you can [`call()`](/isc/schema/how-tos/call) or
[`post()`](/isc/schema/how-tos/post) smart contract function requests. We also created a few wrapper
...
SoloAgent a new _Tangle_ address is created for that agent and its on-chain account is
...
Next we will show how to test [Views](/isc/schema/how-tos/views) and/or [Funcs](/isc/schema/how-tos/funcs that return
a result. Note that even though Funcs are usually invoked through a [`post()`](/isc/schema/how-tos/post)
request, in which case any return values are inaccessible, they still can be invoked
through a [call()](/isc/schema/how-tos/call) from within another Func, in which these return values can
be used normally. Since solo executes [`post()`](/isc/schema/how-tos/post) requests synchronously, it is
