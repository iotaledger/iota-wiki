...
[Schema Tool](../how-tos/wasm/usage.mdx)) will attempt to use the closest matching built-in data type when
...
The [Schema Tool](../how-tos/wasm/usage.mdx) will automatically generate the proper (im)mutable proxies
from the schema definition. For example, View functions will only be able to access the
[State](../how-tos/wasm/state.mdx) map through immutable proxies. The same goes for the
[Params](../how-tos/wasm/params.mdx) map that was passed into a Func or View, and for the
[Results](../how-tos/wasm/results.mdx) map that was returned from a call to a Func or View.
