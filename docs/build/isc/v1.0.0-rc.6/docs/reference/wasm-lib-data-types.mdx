...
[Schema Tool](../schema/how-tos/usage.mdx) will attempt to use the closest matching built-in data type when
...
The [Schema Tool](../schema/how-tos/usage.mdx) will automatically generate the proper immutable proxies
from the schema definition. For example, View functions will only be able to access the
[State](../schema/how-tos/state.mdx) map through immutable proxies. The same goes for the
[Params](../schema/how-tos/params.mdx) map that was passed into a Func or View, and for the
[Results](../schema/how-tos/results.mdx) map that was returned from a call to a Func or View.
