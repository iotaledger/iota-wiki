...
[Schema Tool](/isc/schema/how-tos/usage) will attempt to use the closest matching built-in data type when
...
The [Schema Tool](/isc/schema/how-tos/usage) will automatically generate the proper (im)mutable proxies
from the schema definition. For example, View functions will only be able to access the
[State](/isc/schema/how-tos/state) map through immutable proxies. The same goes for the
[Params](/isc/schema/how-tos/params) map that was passed into a Func or View, and for the
[Results](/isc/schema/how-tos/results) map that was returned from a call to a Func or View.
