...
An emitter is used for filling corresponding values into templates under `tools/isc/schema/generator`.
...
Is the current processed field in basetype? `basetype`s are defined in the map `FieldTypes` in `tools/isc/schema/model/field.go`.
...
Here `else` is a predefined empty template, which is defined at[`tools/isc/schema/generator/templates.go`.
