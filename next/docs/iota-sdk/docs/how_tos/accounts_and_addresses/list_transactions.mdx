---
title: List Transactions
description: 'How to list transactions'
image: /img/logo/iota_mark_light.png
keywords:
- how to
- list transactions
- nodejs
- python
- rust
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";

This How-To will show you how to list all transactions of an account.

:::tip Sync Options

If you want to list incoming transactions you need to specify that in your `sync` call. 
Transactions for outputs that you synced before with other sync options, will not be requested later.
Also only transactions that weren't pruned on the point of syncing will be listed.

:::tip

## Code Example

<Tabs groupId="language">
<TabItem value="rust" label="Rust">

```rust reference
https://github.com/iotaledger/iota-sdk/tree/develop/sdk/examples/how_tos/accounts_and_addresses/list_transactions.rs
```

</TabItem>
<TabItem value="nodejs" label="Nodejs">

```typescript reference
https://github.com/iotaledger/iota-sdk/tree/develop/bindings/nodejs/examples/how_tos/accounts_and_addresses/list-transactions.ts
```

</TabItem>
<TabItem value="python" label="Python">

```python reference
https://github.com/iotaledger/iota-sdk/tree/develop/bindings/python/examples/how_tos/accounts_and_addresses/list_transactions.py
```

</TabItem>
</Tabs>

## Expected Output

```plaintext
Sent transactions:
0x3b92c80c5d108b0b756989aafe8843b98f1981726e5ddfe70bb51bc8e0ad5dbc
Received transactions:

```
