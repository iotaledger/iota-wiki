...
3. Sync the account with the node to get the latest transactions by calling the [`Account.sync()`](https://docs.rs/iota-sdk/latest/iota_sdk/wallet/account/struct.Account.html#method.sync)
   function.
4. Search the account's balance for a foundry.
5. Display the number of foundries in the balance before destroying.
6. Burn the foundry by calling the [`Account.burn()`](https://docs.rs/iota-sdk/latest/iota_sdk/wallet/account/struct.Account.html#method.burn)
   function.
7. Display the number of foundries in the balance after destroying.
...
4. Search the account's balance for a foundry.
5. Display the number of foundries in the balance before destroying.
6. Prepare the transaction that will burn the tokens using the [`Account.prepareDestroyFoundry()`](https://wiki.iota.org/next/iota-sdk/references/nodejs/classes/Account/#preparedestroyfoundry)
   function.
7. Send the prepared transaction using the [`Account.send()`](https://wiki.iota.org/next/iota-sdk/references/nodejs/classes/Account/#send)
   function.
8. Sync the account again, and display the number of foundries in the balance after destroying.
...
4. Search the account's balance for a foundry.
5. Display the number of foundries in the balance before destroying.
6. Prepare the transaction that will burn the tokens using the [`Account.prepare_destroy_foundry()`](https://wiki.iota.org/next/iota-sdk/references/python/iota_sdk/wallet/account/#prepare_destroy_foundry)
   function.
