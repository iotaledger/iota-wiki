...
3. Sync the account with the node to get the latest transactions by calling the [`Account.sync()`](../../references/nodejs/classes/Account/#sync)
   function.
4. Display the current native token balance.
5. Prepare to melt `MELT_AMOUNT` native tokens transaction by calling the [`Account.prepareMeltNativeToken()`](../../references/nodejs/classes/Account/#preparemeltnativetoken)
   function.
...
3. Sync the account with the node to get the latest transactions by calling the [`Account.sync()`](../../references/python/iota_sdk/wallet/account/#sync)
   function.
4. Display the current native token balance.
5. Prepare to melt `MELT_AMOUNT` native tokens transaction by calling the [`Account.prepare_melt_native_token()`](../../references/python/iota_sdk/wallet/account/#prepare_melt_native_token)
   function.
