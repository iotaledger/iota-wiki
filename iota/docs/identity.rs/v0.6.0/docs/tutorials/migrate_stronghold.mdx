...
To migrate a v1 Stronghold Snapshot from Identity 0.6.2 or earlier in Rust to a v2 Stronghold Snapshot that can be used with Identity 0.6.3 Rust, follow these steps:
...
To migrate a v1 Stronghold Snapshot from Identity 0.6.0 or earlier in Wasm to a v2 Stronghold Snapshot that can be used with Identity Wasm 0.6.1, follows these steps. Note that we're using the iota-sdk nodejs version for this process.
