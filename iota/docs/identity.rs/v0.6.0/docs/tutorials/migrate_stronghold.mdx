...
description: Provides instructions on how to migrate Stronghold Snapshot files from v1 to v2
...
With the Rust 0.6.3 and Wasm 0.6.1 version of IOTA Identity, Stronghold was updated to [v2](https://github.com/iotaledger/stronghold.rs/releases/tag/iota-stronghold-v2.0.0). The new Stronghold release, besides many security enhancements, introduces an updated Snapshot format and requires old Snapshots to be migrated to the new format.

:::info
Stronghold Snapshots v1 must be migrated to v2 before they can be used with Identity Rust 0.6.3 and Wasm 0.6.1.
:::

To migrate Stronghold Snapshot files from the v1 to the v2 format the following instructions using IOTA SDK for Rust or Nodejs can be utilized.
...
To migrate a v1 Stronghold Snapshot from Identity 0.6.2 or earlier in Rust to a v2 Stronghold Snapshot, follow these steps:
...
- The program takes 4 arguments, 2 of them required.
    ...
    - Optional: The password of the migrated snapshot.
...
To migrate a v1 Stronghold Snapshot from Identity 0.6.0 or earlier in Wasm to a v2 Stronghold Snapshot, follows these steps. Note that we're using the iota-sdk nodejs version for this process.
...
async function migrate() {
  ...
  if (!currentPath) {
    console.log("expected path to the current stronghold as the first argument");
    process.exit(0)
  }

  if (!currentPassword) {
    console.log("expected password of the current stronghold as the second argument");
    process.exit(0)
  }
  ...
  if (!newPath) {
    const answer = readline.question("WARNING: The current stronghold snapshot will be overwritten, since no new path was given. Enter `yes` to continue\n")
  ...
  if (!newPassword) {
    console.log("NOTE: The password of the new stronghold will be the same as the current one.");
