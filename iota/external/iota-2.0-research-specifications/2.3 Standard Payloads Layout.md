...
The `Deal messages` exchanged to produce a public/private collective key during the DKG phase (Table 2.3.9). The `Deal messages` are issued by the nodes that qualified for the dRNG committee participation <!--(see [Section 6.5 - Distributed Random Number Generator](./6.6dRNG))-->.
